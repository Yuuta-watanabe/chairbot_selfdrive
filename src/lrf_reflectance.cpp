...
void laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan) {
  ...
  *pcl_cloud = *pc_tmp2;
  ...
  ec.setClusterTolerance(0.4);  // 5cm
  ...
  int id= 0;//
  for(int i_=0;i_<cluster_indices.size();i_++){//

  pcl::PointCloud<pcl::PointXYZ>::Ptr pc_tmp(new pcl::PointCloud<pcl::PointXYZ>);
  // pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_cloud (new pcl::PointCloud<pcl::PointXYZ>);
  for (std::vector<int>::const_iterator pit = cluster_indices.at(i_).indices.begin();
       pit != cluster_indices.at(i_).indices.end(); ++pit) {
  ...
  *pcl_cloud = *pc_tmp;
  ...
  std::cout << "pcl_cloud size " <<  pcl_cloud->size() << std::endl;
  // visualization_msgs::MarkerArray markerArray;
  // int id= 0;
  ...
  //
  //   marker_circle.pose.position.x =  iter->x;
  //   marker_circle.pose.position.y =  iter->y;
  //   marker_circle.pose.position.z =  iter->z;
  ...
  pose.pose.position.x = (float)pcl_cloud->points[pcl_cloud->size()/2 - 2].x;
  pose.pose.position.y = (float)pcl_cloud->points[pcl_cloud->size()/2 - 2].y;
  pose.pose.position.z = (float)pcl_cloud->points[pcl_cloud->size()/2 - 2].z;
  ...
  marker_circle.pose.position.x =  pose.pose.position.x;
  marker_circle.pose.position.y =  pose.pose.position.y;
  marker_circle.pose.position.z =  pose.pose.position.z;
  marker_circle.pose.orientation.x = 0.0;
  marker_circle.pose.orientation.y = 0.0;
  marker_circle.pose.orientation.z = 0.0;
  marker_circle.pose.orientation.w = 1.0;
...
int main(int argc, char** argv) {
  ...
  pub___ = nh.advertise< visualization_msgs::MarkerArray >("visualization_marker_array", 1);
